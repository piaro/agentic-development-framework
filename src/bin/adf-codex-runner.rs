use adf::codex_runner::{CodexRunnerOptions, run};
use std::env;
...
fn parse(arguments: &[String]) -> Result<CodexRunnerOptions, String> {
    ...
    while index < arguments.len() {
        ...
        match flag {
            ...
            "--codex-bin" => codex_binary = PathBuf::from(value),
            "--model" => model = Some(value),
            other => return Err(format!("unknown argument: {other}")),
        }
        index += 1;
    }
    Ok(CodexRunnerOptions {
...
fn print_usage() {
    println!(
        "usage: adf-codex-runner run --project <root> --change <id> --expected-action <id> --expected-context <digest> [--adf-bin <path>] [--codex-bin <path>] [--model <name>]"
    );
}
