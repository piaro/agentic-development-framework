...
When this context produced the analysis or implementation and an optional agent runner
is available, the orchestrating agent may invoke `adf-codex-runner` or
`adf-claude-runner` with the exact Change ID, Action ID, and Context digest returned by
`adf_next`. Each runner starts one fresh, non-persistent Challenger execution and returns
control after that Action. A runner is an execution option, not authority: do not pass
it a summary of the producing conversation or use that summary to supplement the
generated ADF Context.
