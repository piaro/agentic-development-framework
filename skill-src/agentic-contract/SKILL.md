...
4. Resolve existing governing contracts. Identify missing, proposed, contradictory, or stale contracts.
5. For data changes, require applicable Data Invariants and Operation Contracts. Define transaction groups, consistency, idempotency, failure points, concurrency, deletion, and external effects.
6. Separate platform assumptions from verified capabilities. Add required probes for unverified dependencies.
7. Write `.agentic/changes/<id>/contract-assessment.yaml` with governing contracts, decisions, gaps, conflicts, platform unknowns, and result.
8. If an upper contract is missing, set the change to `blocked-contract-decision`, propose the contract and options, and obtain the required human decision before accepting it.
9. When all decisions are resolved, all required contracts are accepted, and probes exist, set assessment `result: ready` and run `agentic contract lint`.
