...
自動テストでは、同じ論理内容のYAML群とMarkdown群から同じProject Snapshot digestを生成する。人向け本文を追加してもdigestが変わらず、typed blockのclauseを変更するとdigestが変わる。また、Human判断をDecisionとContractへ反映したときも既存の人向け本文が保持され、新しいDecisionがMarkdownとして作成されることを確認する。Shared Contractについては、同じdigestを読んだ二つの更新のうち先に保存された一件だけを受理し、後続のstale更新が正本を上書きしないことをRust版のgolden fixtureで確認する。
